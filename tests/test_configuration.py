...
class TestInstrumentConfiguration(unittest.TestCase):
    ...
    def test_cache(self):
        ...
        # test that accessing channel_mask changes cache from False to True
        self.assertFalse(hasattr(config, "cache"))
        self.assertEqual(config.channel_mask.size, 4)
        self.assertTrue(hasattr(config, "cache"))

        # test that adding detector updates cache and increases mask size 
        config.add_detector("test", config.settings["detectors"]["t0"])
        self.assertEqual(config.channel_mask.size, 6)
